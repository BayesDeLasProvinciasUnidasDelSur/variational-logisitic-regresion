...
%
\en{First we take the log of the logistic function and then decompose it so that}
%
\begin{equation}
\begin{split}
 f(x) = \log \sigma(x) & = - \log(1+e^{-x}) = - \log(e^{-x/2}(e^{x/2}+e^{-x/2})) \\
 & = x/2 - \log(e^{x/2}+e^{-x/2})
\end{split}
\end{equation}
%
\en{We now note that the function $f(x)$ is a convex function of the variable $x^2$, as can again be verified by finding the second derivative.}
%
\en{But we can also be verified by noting that $\sigma(x^2)$ is convex, and also the $\log \sigma(x^2)$ }
%
\begin{figure}[H]
\centering
\begin{tikzpicture}
...
\end{figure}
%
\en{We compute the derivative of $f(x) = - \log(e^{x/2}+e^{-x/2})$ with respect to x}
%
\begin{equation}
\frac{df}{dx} = -\frac{1}{e^{x/2}+e^{-x/2}} \cdot \frac{1}{2} e^{x/2}\cdot -\frac{1}{2} e^{-x/2}
\end{equation}
